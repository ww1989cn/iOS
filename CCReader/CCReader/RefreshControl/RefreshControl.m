...
- (void)initTopView
{
    ...
    if (!CGRectIsEmpty(self.scrollView.frame))
    {
        if (self.topView==nil)
        {
            ...
            _topView=[[className alloc] initWithFrame:CGRectMake(0, NAV_HEIGHT-100, self.scrollView.frame.size.width, self.enableInsetTop+35)];
            [self.scrollView addSubview:self.topView];
        }
        else{
            _topView.frame=CGRectMake(0, NAV_HEIGHT-100, self.scrollView.frame.size.width, self.enableInsetTop+35);
